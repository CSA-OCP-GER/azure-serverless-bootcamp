...
>Hint:
You can install all of these needed software by using **winget** (https://docs.microsoft.com/en-us/windows/package-manager/winget/)
## Challenge
...
2. Choose a runtime if you are prompted, and select "JavaScript" for the language
3. Click the lightning bolt icon in the Azure Functions extension to add a function to this app.  Select **HTTP Trigger** for the trigger.  Give it any name you like (I'll name "products")
4. Select **Anonymous** for the authentication type.  **Function** would also work but requires a key is passed in a header or query parameter to execute the function once published.
5. You should now see a default Azure Functions template like the following:
...
6. Make the following changes so that your function returns the suggested string:
...
7. Click **Run** at the top and **Start Debugging**
...
8. While the runtime is still running, click on the link or copy it to a browser to execute the function.  Make sure you append a query parameter for ID as specified.  So the call should be like `http://localhost:7071/api/products?id=1`.  You should see a response like the following returned:
...
9. The final step is publishing this app to Azure.  Kill the terminal (click the trash icon) to stop the runtime, and open the Azure Functions extension.
10. Click the **"deploy to azure"** icon in the Functions extension to publish, and select this app in the list.  Choose to **Create New Function App** and give it a *globally* unique name and follow the dialog.
